 ...
 * 
 * Updated by DanCake to include CakePHP 1.3,
 * component & beforeFilter support.
 * 
...
class EventComponent extends Object {
	...
	private function _dispatchEvent($scope, $eventName, $data = array()) {
		...
		foreach($validHandlersClasses as $class) {
			...
			if ($class != get_class($controller)) {
				...
				foreach(array_keys($controller->components) as $component) {
					if (is_callable(array($controller->$component, 'initialize'))) {
						if($component == 'Cookie') {
							// CookieComponent has a mandatory settings param
							$controller->$component->initialize($controller, array());
						} else {
				...
				// Call the beforeFilter method if it exists
				if (is_callable(array($controller, 'beforeFilter'))) {
					call_user_method_array('beforeFilter', $controller, array());
				}
			}
	...
	private function _scanControllers($scope, $eventHandlerMethod) {
		...
		foreach ($controllers as $controller) {
			// FIX: Ignore the pages controller, causes a fatal error
			if (($scope == $controller || low($scope) == 'global') && $controller != 'Pages') { // must be in scope, or global
