 ...
 */
	var $steps = array();
/**
 * The original steps array that was initially passed to the wizard.
 *
 * @var array
 * @access public
 */
	var $stepsOriginal = array();
/**
 ...
 */
	function startup(&$controller) {
		...
		//Backup the steps array
		$this->stepsOriginal = $this->steps;
 ...
 */
	...
	function write($key, $value) {
		$this->Session->write("$this->_sessionKey.$key", $value);
	}
/**
...
/**
 * Regenerates the steps array using the original.
 * Prevents the wrong branch from being used.
 *
 * @access public
 */
 ...
 */
	function _validStep($step) {
		if ($this->in_multi_array($step, $this->steps)) {
			if ($this->lockdown) {
	...
	function in_multi_array($needle, $haystack) {
		foreach ($haystack as $val) {
			if (is_array($val)) {
				if (in_multi_array($needle, $val)) return true;
			} else if ($val == $needle) {
				return true;
			}
		}
