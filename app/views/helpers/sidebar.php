	...
	function getSidebar() {
		...
		if ($this->enabled()) {
			...
			foreach($sidebox_elements as $sb) {
				...
				if (!empty($sb['more'])) {
					if(empty($sb['more']['title'])) $sb['more']['title'] = 'More';
					$box_output .= $this->Html->link($sb['more']['title'], $sb['more']['url'], array('class' => 'more'));
				}
				...
				if (!empty($sb['content'])) {
					...
					$box_output .= $view->element($sb['element'], $sb['params']);
				}

				// wrap it all in a div
				$boxOptions = array();
				if(!empty($sb['id'])) $boxOptions['id'] = $sb['id'];
				$boxOptions['class'] = !empty($sb['class']) ? $this->options['sidebox_class'] . ' ' . $sb['class']: $this->options['sidebox_class'];
				$box_output = $this->Html->tag('div', $box_output, $boxOptions);

				$output .= $box_output;
			}

			$sidebarOptions = array();
			//if(!empty($this->options['sidebar_id'])) $sidebarOptions['id'] = $this->options['sidebar_id'];
			//if(!empty($this->options['sidebar_class'])) $sidebarOptions['class'] = $this->options['sidebar_class'];
			//$output = $this->Html->tag('div', $output, $sidebarOptions);
		}
